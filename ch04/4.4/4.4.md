...
6. grammar for boolean expressions:
    ...
    ```
    Notice that bterm appears in bodies only followed by bexpr'.
    Thus, everything except ε that in FIRST(bexpr') must be in FOLLOW(bterm);that explains the symbol or.
    However, since FIRST(bexpr') contains ε, and bexpr' is the entire string following bterm in the bodies
    of the bexpr-productions, everything in FOLLOW(bexpr) must also be in FOLLOW(bterm).
    That explains the symbol $ and ).
    ```
